...
public class StaffUpdate extends javax.swing.JPanel {
    ...
    public StaffUpdate(String nic) {
        try {
            System.out.println(nic);
            ResultSet staffupdaters = connection.execute("SELECT user.*, user_has_address.*, city.*, district.*, province.*, gender.* FROM `user` INNER JOIN `user_has_address` ON `user`.`nic` = `user_has_address`.`user_nic` INNER JOIN `city` ON `city`.`city_id` = `user_has_address`.`city_city_id` INNER JOIN `district` ON `district`.`did` = `city`.`district_id` INNER JOIN `province` ON `province`.`pid` = `district`.`province_id` INNER JOIN `gender` ON `user`.`gender_id` = `gender`.`g_id` WHERE `user`.`nic` ='" + nic + "'");

            if (staffupdaters.next()) {
                System.out.println(staffupdaters.getString("fname"));
                jTextField1.setText(staffupdaters.getString("fname"));
                jTextField4.setText(staffupdaters.getString("lname"));
                jTextField5.setText(staffupdaters.getString("nic"));
                jTextField2.setText(staffupdaters.getString("mobile"));
                jTextField6.setText(staffupdaters.getString("email"));
                jPasswordField1.setText(staffupdaters.getString("password"));
                jTextField3.setText(staffupdaters.getString("line_1"));
                jTextField7.setText(staffupdaters.getString("line_2"));
//                jComboBox1.setSelectedItem(staffupdaters.getString("g_name"));
//                jComboBox2.setSelectedItem(staffupdaters.getString("p_name"));
//                jComboBox3.setSelectedItem(staffupdaters.getString("d_name"));
//                jComboBox4.setSelectedItem(staffupdaters.getString("city_name"));
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Update Academic Staff");
        ...
        jToggleButton1.setFont(jToggleButton1.getFont().deriveFont(jToggleButton1.getFont().getStyle() | java.awt.Font.BOLD, jToggleButton1.getFont().getSize()+1));
            ...
            .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                .addGroup(layout.createSequentialGroup()
                    ...
                    .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                        ...
                        .addComponent(jPanel6, javax.swing.GroupLayout.PREFERRED_SIZE, 1125, javax.swing.GroupLayout.PREFERRED_SIZE))
                    .addGap(0, 0, Short.MAX_VALUE)))
        );
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        if (firstname.isEmpty()) {
            ...
            try {
                ...
                //still not completed
                ResultSet rs = connection.execute("UPDATE `user` SET `password` = '" + password + "',`fname` = '" + firstname + "', `lname` = '" + lastname + "',`mobile` = '" + mobile + "', `email` = '" + email + "', `gender_id` = '" + genderId + "', `status_st_id` = '" + 2 + "' WHERE `nic`='" + nic + "'");

                JOptionPane.showMessageDialog(jPanel2, "User Updated Successfully !");
